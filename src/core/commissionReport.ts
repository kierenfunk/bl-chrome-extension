...
const commissionReport = (data: ConnCommsRcti[], lastPeriod: boolean = false) : string => {
    ...
    const globalData: GlobalData = { 
        latestDate: Array.from(accountNames).reduce((obj,accountName)=>{
            return {...obj, [accountName]: Math.max(...data.filter(item=>item.accountName===accountName).map(item=>item.endDate))}
        },{}),
        periods: Array.from(accountNames).reduce((obj,accountName)=>{
            return {...obj, [accountName]: Array.from(new Set(data.filter(item=>item.accountName===accountName).map(item=>item.endDate))).sort()}
        },{}),
        lastPeriod
    }
    ...
    const y: DictifiedComms = Object.values(dataMap).reduce((result,values)=>{
