...
const buildTable = (header: [string, string][], data: any) => {
    ...
    result += data.data.map((row: any)=>{
        return '\t<tr>\n'+header.map(([key,])=>{
            return `\t\t<td>${convertValue(row[key], key)}</td>`
        }).join("\n")+'\n\t</tr>\n'
    }).join('')
    result += '</tbody>\n</table>\n'
    return result
...
const getCommissionItemValues = (data: CommissionItemWrap, key: string) : string[] | boolean | CommissionItem[] => {
    ...
    else if(key === 'discontinued') return data.discontinued
...
const descend = (dataMap: any, retrieveKey: string, height: number =2): any => {
    ...
    return Object.values(dataMap).reduce((arr: any,value)=>{
        return [...arr, ...descend(value, retrieveKey, height-1)]
    },[])
}
